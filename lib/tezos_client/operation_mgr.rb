...
class TezosClient
  class OperationMgr
    ...
    def ensure_applied!(rpc_responses)
      ...
      internal_operations = metadatas.map { |metadata| metadata[:internal_operations] }.compact
      operation_results += internal_operations.map { |internal_operations| internal_operations[:result] }
